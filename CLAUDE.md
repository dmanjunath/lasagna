...
1. **Question the pattern first, not just the styling.** Is this even the right control/placement, or is a better-known pattern (how ChatGPT/Claude/Linear solve it) being ignored? A well-tokenized *wrong pattern* still fails. Decide this before writing CSS.
2. **Match the design system** (per the paragraph above) — reuse the uikit primitive / `--ui-*` tokens; never a raw native control or foreign color/radius.
3. **Design and render EVERY state, then drive the interactions** — resting *and* hover, focus, active/selected, disabled, empty, loading, error, and for menus/modals the **open/expanded** state; plus long/overflow data, **light + dark**, **desktop + mobile**. Don't screenshot a resting state and call it working: open it, **scroll it**, type, submit, resize, click every affordance.
4. **Then dispatch the `ui-reviewer` agent to evaluate the rendered result** (`Agent` tool, `subagent_type: "ui-reviewer"`). Give it the route, the changed files, what it does, and the current `lasagna_session` token. **Do NOT ask the user to eyeball it** — the agent is the gate. Fix every finding it returns and **re-invoke it until it returns PASS**. Only a PASS means done. If a finding is genuinely a separate feature that expands scope, surface it to the user with a recommendation rather than silently building it.
