...
export async function detectRecurringForTenant(tenantId: string): Promise<{
  ...
  // descrubOutput: false — the response is JSON.parsed below, and a real name
...
${JSON.stringify(compact)}`,
  ...
  try {
    ...
    // Restore real names in the user-visible fields (the model saw aliases).
    // merchantName must be restored BEFORE resolveAccountId matches it against
    // real transaction merchant strings.
    parsed = parsed.map((r) => ({
      ...r,
      name: typeof r.name === "string" ? descrub(r.name, aliasMap) : r.name,
      merchantName: typeof r.merchantName === "string" ? descrub(r.merchantName, aliasMap) : r.merchantName,
      reasoning: typeof r.reasoning === "string" ? descrub(r.reasoning, aliasMap) : r.reasoning,
    }));
  } catch (e) {
