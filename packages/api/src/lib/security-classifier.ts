 ...
 *
 * Pass a prebuilt aliasMap when calling in a loop so the llm boundary doesn't
 * rebuild the tenant's alias map (3 DB queries) per symbol.
 */
...
export async function classifyUnknownSecuritiesForTenant(
  ...
  try {
    ...
    // Build the tenant alias map ONCE for the whole batch — each model call
    // would otherwise rebuild it (3 DB queries) per symbol.
    const aliasMap = await buildAliasMap(tenantId);

    for (const symbol of todo) {
      const sec = bySymbol.get(symbol)!;
      const classified = await classifySecurity(tenantId, {
        symbol,
        name: sec.name,
