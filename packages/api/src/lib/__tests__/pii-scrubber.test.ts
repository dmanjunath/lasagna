...
describe("scrub", () => {
  ...
  it("does not replace inside larger words (word boundaries)", () => {
    // "Sam" mapped as a person part must not mangle "Samsung".
    const partMap = makeMap([["Sam", "Person A"]]);
    const out = scrub("Sam bought a Samsung TV from Samsung.", partMap);
    expect(out).toBe("Person A bought a Samsung TV from Samsung.");
  });

  it("still strips needles that start/end with non-word chars (masks like ****1234)", () => {
    const maskMap = makeMap([["****1234", ""]]);
    const out = scrub("Card ****1234 was charged.", maskMap);
    expect(out).toBe("Card  was charged.");
  });
...
describe("descrub", () => {
  ...
  it("does not expand aliases embedded inside larger words", () => {
    // A bare debt alias like "auto" must not re-expand inside other words.
    const bareMap = makeMap([["Wells Fargo Auto Loan", "auto"]]);
    const out = descrub("Set to automatic payments for your auto.", bareMap);
    expect(out).toBe("Set to automatic payments for your Wells Fargo Auto Loan.");
  });
...
describe("person + address anonymization", () => {
  // Import the pure helpers directly (no DB).
  it("aliases the full name (round-trip) and strips name parts + email", async () => {
    const { addPersonToMap, scrub, descrub } = await import("../pii-scrubber.js");
    const map = { forward: new Map<string, string>(), reverse: new Map<string, string>() };
    addPersonToMap(map, "Person A", "Jordan Rivera", "jordan.rivera1@example.com");

    const out = scrub(
      "Prepared for Jordan Rivera. Contact jordan.rivera1@example.com. Jordan holds the account.",
      map,
    ) as string;
    expect(out).not.toContain("Jordan");
    expect(out).not.toContain("Rivera");
    expect(out).not.toContain("jordan.rivera1@example.com");
    expect(out).toContain("Person A");

    // Model echoes the alias → display restores the full name.
    expect(descrub("A note for Person A.", map)).toContain("Jordan Rivera");
  });

  it("skips short middle initials to avoid collisions", async () => {
    const { addPersonToMap, scrub } = await import("../pii-scrubber.js");
    const map = { forward: new Map<string, string>(), reverse: new Map<string, string>() };
    addPersonToMap(map, "Person B", "Alex Q Rivera", null);
    // "R " prose must not be mangled; the real parts must be gone.
    const out = scrub("Alex Q Rivera and the letter Q remain.", map) as string;
    expect(out).not.toContain("Alex");
    expect(out).toContain("letter Q remain");
  });

  it("does not map common financial words as standalone name parts", async () => {
    const { addPersonToMap, scrub, descrub } = await import("../pii-scrubber.js");
    const map = { forward: new Map<string, string>(), reverse: new Map<string, string>() };
    // A person whose name collides with financial vocabulary: the FULL name
    // still round-trips, but the common-word part must not become a standalone
    // strip that mangles ordinary prose.
    addPersonToMap(map, "Person A", "John Bank", null);

    expect(descrub(scrub("John Bank opened an account.", map) as string, map)).toContain("John Bank");
    const out = scrub("Your bank statement from Chase Bank arrived.", map) as string;
    expect(out).toBe("Your bank statement from Chase Bank arrived.");
    // The non-common part is still stripped standalone.
    expect(scrub("John holds the account.", map)).toBe("Person A holds the account.");
  });

  it("strips property addresses and street segments, never restoring them", async () => {
    const { addPropertyAddressToMap, scrub, descrub } = await import("../pii-scrubber.js");
    const map = { forward: new Map<string, string>(), reverse: new Map<string, string>() };
    addPropertyAddressToMap(map, "412 Maplewood Ct, Springfield, VA 22150", 1);

    const out = scrub(
      "The rental at 412 Maplewood Ct, Springfield, VA 22150 (aka 412 Maplewood Ct) cash-flows.",
      map,
    ) as string;
    expect(out).not.toContain("Maplewood");
    expect(out).toContain("the property 1 address");
    // Forward-only: descrub must NOT reintroduce the address.
    expect(descrub(out, map)).not.toContain("Maplewood");
  });

  it("nulls geocode identity fields (placeId/lat/lng) like masks", async () => {
    const { scrub } = await import("../pii-scrubber.js");
    const map = { forward: new Map<string, string>(), reverse: new Map<string, string>() };
    const out = scrub(
      { metadata: { placeId: "ChIJabc", lat: 38.84, lng: -77.44, monthlyRent: 2600 } },
      map,
    ) as { metadata: { placeId: null; lat: null; lng: null; monthlyRent: number } };
    expect(out.metadata.placeId).toBeNull();
    expect(out.metadata.lat).toBeNull();
    expect(out.metadata.lng).toBeNull();
    expect(out.metadata.monthlyRent).toBe(2600);
  });
});

describe("harvesting identity from account names", () => {
  it("extracts person names from aggregator-style account titles", async () => {
    const { harvestPersonNames } = await import("../pii-scrubber.js");
    expect(harvestPersonNames("Jordan Rivera - Brokerage Account - ****9012")).toEqual([
      "Jordan Rivera",
    ]);
    expect(
      harvestPersonNames("Alex Q Rivera, Jordan Rivera Cust - UTMA Brokerage Account"),
    ).toEqual(["Alex Q Rivera", "Jordan Rivera"]);
    // Non-person titles yield nothing.
    expect(harvestPersonNames("Emergency Fund")).toEqual([]);
    expect(harvestPersonNames("HQ CHECKING")).toEqual([]);
  });

  it("extracts street segments by suffix keyword", async () => {
    const { harvestStreetSegments } = await import("../pii-scrubber.js");
    expect(harvestStreetSegments("Rental — Maplewood Ct, Springfield")).toEqual(["Maplewood Ct"]);
    expect(harvestStreetSegments("Rental — Sunny Brook Ct, Springfield")).toEqual([
      "Sunny Brook Ct",
    ]);
    expect(harvestStreetSegments("Maplewood Ct. Mortgage Loan")).toEqual(["Maplewood Ct"]);
    expect(harvestStreetSegments("Roth IRA Brokerage Account")).toEqual([]);
  });
});
