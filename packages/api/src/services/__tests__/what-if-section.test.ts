...
describe("buildWhatIfSection", () => {
  ...
  it("never pulls the retire-earlier age below the current age", async () => {
    // currentAge 63, retirementAge 65 → 3-years-earlier clamps to 63, not 62.
    ...
    const earlier = section.scenarios.find((s) => s.label.includes("earlier"))!;
    expect(earlier.overrides.retirementAge).toBe(63);
  });
