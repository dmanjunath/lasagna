...
async function runScenario(
  ...
  extraInvestable: number | undefined,
...
export async function buildWhatIfSection(
  ...
  // every run's starting balance, so the base and each scenario reflect the sale.
  extraInvestable?: number,
): Promise<WhatIfSection> {
  const generatedAt = new Date().toISOString();
  const base: SimInputs = await resolveSimInputs(
    tenantId,
    userId,
    baseOverrides,
    flatReturn,
  ...
  // Retire N years earlier, clamped to at least a year from now — an age at or
  // below the current age would be a scenario in the past. When the clamp
  // changes the age, the label names the actual age used; when it leaves no
  // room before the base retirement age, the scenario is skipped entirely.
  const earlierAge = Math.max(base.currentAge + 1, base.retirementAge - RETIRE_EARLIER_YEARS);
  const specs: ScenarioSpec[] = [];
  if (earlierAge < base.retirementAge) {
    specs.push({
      label:
        earlierAge === base.retirementAge - RETIRE_EARLIER_YEARS
          ? `Retire ${RETIRE_EARLIER_YEARS} years earlier`
          : `Retire at ${earlierAge}`,
  ...
  const results = await Promise.all(
    specs.map((spec) =>
      runScenario(tenantId, userId, spec, baseSuccessRate, baseOverrides, flatReturn, extraInvestable),
    ),
  );
