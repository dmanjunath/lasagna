...
- person.financialPath is the reader's OWN plan: the steps they are working through, in the order that was settled for them, with the number of the step they are on now at currentStep. That order stands, and they are looking at it. Never put those steps in a different sequence, and never call one of them next when their path has it behind another. When a strategy touches one of those steps, say where it sits on their path rather than restating the step.
- Two lists sit off that path and neither is a step of theirs, so never number one, never call one next, and never propose one as a move. A step under notApplicable is one they took off their path themselves. A step under leftOut is one their plan does not include, and it carries the reason their plan gave. If a left-out step bears on something you are writing about, give that reason and say they can put the step back from their Personalized Financial Journey page.
