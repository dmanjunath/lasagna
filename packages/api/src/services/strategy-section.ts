...
- person.financialPath is the reader's OWN plan: the steps they are working through, in the order that was settled for them, with the number of the step they are on now at currentStep. That order stands, and they are looking at it. Never put those steps in a different sequence, never call one of them next when their path has it behind another, and never propose a step listed under notApplicable, which they took off their path. When a strategy touches one of those steps, say where it sits on their path rather than restating the step.
