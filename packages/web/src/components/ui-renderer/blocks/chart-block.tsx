...
export function ChartBlockRenderer({ block }: { block: ChartBlockType }) {
  if (block.chartType === "area") {
    return (
      <div className="h-64">
        {block.title && (
          <h4 className="text-[13px] font-bold uppercase tracking-[0.06em] text-content-muted mb-2">
            {block.title}
          </h4>
        )}
        <AreaChart
  ...
  if (block.chartType === "donut") {
    ...
    return (
      <div className="h-64 flex items-center justify-center">
        {block.title && (
          <h4 className="text-[13px] font-bold uppercase tracking-[0.06em] text-content-muted mb-2 absolute top-0 left-0">
            {block.title}
          </h4>
        )}
