...
/**
 * The Retirement Plans list — cards with rename/delete and the name-only
 * "New plan" modal. Self-contained (fetches its own data) so it can render both
 * as the standalone /financial-plans page and inside the Retirement page's
...
export function FinancialPlansList({
   ...
   * recommends generating a plan, so its button lands on the modal itself
   * rather than on a tab that repeats the recommendation.
   */
  autoCreate?: boolean;
  ...
  // The modal hands focus back to whatever opened it, and the retirement page
  // opens it with no control of its own, which drops focus on <body>. Park it
  // on the New plan button of this page instead. Only one of the two buttons
  // is ever mounted, so one ref covers both.
  ...
  // The empty list already recommends generating a first plan, so this surface
  // only ever shows the stale variant.
  const freshness = planFreshness(plans);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- fires on the request, not on every openCreate identity
  useEffect(() => {
  ...
  // Closing the modal without creating: put focus back on the button that
  // opens it, so Escape never leaves the keyboard on <body>.
  ...
  const create = async () => {
    ...
    try {
      ...
      setCreateError(
        e instanceof Error && e.message ? e.message : "Could not create the plan. Try again.",
      );
      setCreating(false);
    }
  };
    ...
    <div>
      ...
      {!loading && error && (
        <EmptyState
          ...
          description="Something went wrong fetching your plans. Please try again."
          action={
      ...
      {!loading && !error && plans.length === 0 && (
        <EmptyState
          icon={<FileText className="h-5 w-5" />}
          title="Generate your first retirement plan"
          description="A written analysis of where you stand and what to do next, built from your real accounts. We recommend starting one now."
          action={
            <Button
              ref={newPlanRef}
      ...
      {!loading && !error && plans.length > 0 && (
        <>
          <PlanFreshnessBanner
            className="mb-5"
            freshness={freshness}
            planName={freshness.newest?.title}
            refresh={{
              label: "Open plan",
              onClick: () => {
                if (freshness.newest) navigate(`/financial-plans/${freshness.newest.id}`);
              },
            }}
          />
      ...
      <Modal
        ...
        title="New plan"
        description="A written analysis of your finances, built from your live accounts. Takes about ten minutes. Browse the app meanwhile and we'll notify you."
        footer={
          <Button onClick={create} disabled={creating || !nameDraft.trim()}>
            {creating ? "Starting…" : "Create plan"}
          </Button>
        }
      >
        <input
