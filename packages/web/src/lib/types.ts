...
// ── Schedule section (deterministic year-by-year drawdown) ───────────────────
// Mirrors ScheduleSection in packages/api/src/services/retirement-schedule.ts.

export type ScheduleBucketKey = "taxable" | "deferred" | "roth" | "hsa";

export type ScheduleBucketFlow = {
  start: number;
  growth: number;
  contribution: number;
  withdrawal: number;
...
export type ScheduleAccountFlow = {
  id: string;
  name: string;
  bucket: ScheduleBucketKey;
  kind: "depository" | "investment";
  earmarked?: boolean;
  start: number;
  growth: number;
  contribution: number;
...
export type ScheduleRow = {
  ...
  year: number;
  phase: "accumulation" | "retirement";
  buckets: Record<ScheduleBucketKey, ScheduleBucketFlow>;
  /** Per-account flows, in draw order. Absent on plans generated before per-account tracking. */
...
// ── Strategy section (LLM, grounded on the schedule) ──────────────────────────
// Mirrors StrategySection in packages/api/src/services/strategy-section.ts.

export type StrategyItem = { title: string; detail: string };
export type StrategyMove = { title: string; detail: string; quantifiedImpact?: string };

export type StrategySection = {
  section: "strategy";
  situationHeadline: string;
  watchouts: StrategyItem[];
  strategies: StrategyMove[];
  explore: StrategyItem[];
  generatedAt: string;
};

// Freeform experiment: the model authors the whole report as one HTML document.
export type FreeformReport = {
  /** Absent on pre-async documents → treat as "ready". */
...
export type FinancialPlanDocument = {
  ...
  // is also absent when the LLM call failed at create time. `schedule` and
  // `strategy` are the current report engine; legacy plans carry narrative /
  // suggestions instead.
  // Absent on freeform plans, which carry `freeform` instead.
  sections?: {
