...
export const api = {
  ...
  createFinancialPlan: (title?: string, type?: "structured" | "freeform") =>
    request<{ plan: FinancialPlan }>("/financial-plans", {
      method: "POST",
      body: JSON.stringify({ ...(title ? { title } : {}), ...(type ? { type } : {}) }),
    }),

  // Freeform experiment: send feedback; the model revises its own report in the
  // background (response returns immediately with status "revising").
  sendPlanFeedback: (id: string, feedback: string) =>
    request<{ document: FinancialPlanDocument }>(`/financial-plans/${id}/feedback`, {
      method: "POST",
      body: JSON.stringify({ feedback }),
    }),

  // Re-kick a failed freeform generation.
  regenerateFreeformReport: (id: string) =>
    request<{ document: FinancialPlanDocument }>(`/financial-plans/${id}/regenerate`, {
      method: "POST",
    }),

  renameFinancialPlan: (id: string, title: string) =>
    request<{ id: string; title: string }>(`/financial-plans/${id}`, {
      method: "PATCH",
