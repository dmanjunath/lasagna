...
async function main() {
  ...
  // Chat (open chat panel on mobile by clicking the floating button)
  await mPage.goto(`${BASE}/`);
  await mPage.waitForTimeout(3000);
  // Try to click the chat floating pill button
  const chatBtn = mPage.locator('[aria-label="Open chat"], button:has-text("Chat"), .chat-pill');
  if (await chatBtn.count() > 0) {
    await chatBtn.first().click();
    await mPage.waitForTimeout(1500);
  }
  await shot(mPage, "mobile-chat");
